...
export class AdminController {
  ...
  cards=async(req:Request,res:Response)=>{const actor=this.#authenticate(req,false);await this.#rbac.assert(actor.userPublicId,'users.read');const query=typeof req.query.q==='string'?req.query.q.trim():'';res.setHeader('Cache-Control','no-store');res.json({success:true,message:'Cards retrieved.',data:await this.#service.listCards(query)});};
