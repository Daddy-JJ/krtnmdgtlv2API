...
export type AdminCard=Readonly<{publicId:string;slug:string;fullName:string;contactEmail:string;ownerEmail:string|null;planCode:string;status:string;createdAt:Date}>;
export type AdminActivity=Readonly<{event:string;actorEmail:string|null;metadata:string|null;createdAt:Date}>;
export interface AdminRepository{listPlans():Promise<AdminPlan[]>;updatePlan(actorPublicId:string,code:'basic'|'pro',input:AdminPlanInput,now:Date):Promise<AdminPlan|null>;listPayments(limit:number):Promise<AdminPayment[]>;listUsers(limit:number):Promise<AdminUser[]>;listCards(limit:number,query:string):Promise<AdminCard[]>;listThemes():Promise<AdminTheme[]>;updateTheme(actorPublicId:string,code:string,input:AdminThemeInput,now:Date):Promise<AdminTheme|null>;listActivity(limit:number):Promise<AdminActivity[]>;}
