...
export class CardService {
  ...
  #response(card: OwnedCard): CardResponse { const { id: _id,logoPath, ...data } = card;return { ...data, canonicalUrl: `${this.#appUrl}/${card.slug}`, qrImageUrl: `/api/v1/public/cards/${encodeURIComponent(card.slug)}/qr`,logoUrl:card.planCode==='pro'&&logoPath?`/api/v1/public/cards/${encodeURIComponent(card.slug)}/logo`:null,whatsappUrl:card.planCode==='pro'?this.#whatsappUrl(card.contact.mobilePhone):null }; }
  #isPublicSlug(value:string):boolean{return /^[A-Za-z]{7}$/.test(value)||/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(value)&&value.length>=3&&value.length<=100;}
