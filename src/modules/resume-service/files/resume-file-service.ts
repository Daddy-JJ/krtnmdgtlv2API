...
import { ClamAvResumeScanner, type ResumeMalwareScanner } from './resume-malware-scanner.ts';

export class ResumeFileService{
  readonly pool:Pool;readonly storage:ResumePrivateStorage;readonly rbac:RbacService;
  readonly scanner: ResumeMalwareScanner;
  constructor(pool:Pool,storage:ResumePrivateStorage,rbac:RbacService,scanner:ResumeMalwareScanner=new ClamAvResumeScanner()){this.pool=pool;this.storage=storage;this.rbac=rbac;this.scanner=scanner;}
  async upload(actor:string,requestPublicId:string,role:ResumeFileRole,file:{originalname:string;buffer:Buffer}){
    ...
    try{
      await this.scanner.assertClean(stored.storagePath);
      metadata.scanStatus='CLEAN_ANTIVIRUS';
      const [users]=await this.pool.execute<Array<RowDataPacket&{id:number}>>(`SELECT id FROM users WHERE public_id=? AND status='active' LIMIT 1`,[actor]);
      if(!users[0])throw new AppError(401,'AUTH_REQUIRED','Authentication is required.');
  ...
  async downloadCurrent(actor:string,requestPublicId:string){
    ...
    if(row.scan_status!=='CLEAN_ANTIVIRUS')throw new AppError(422,'RESUME_FILE_UNSAFE','Resume file has not passed validation.');
